...
In order to achieve a close match between \mf and MT3D-USGS, the XT3D solver was activated within \mf.  Moreover, both solutions use their respective TVD advection schemes.  Both models are also given the same effective molecular diffusion coefficient, which matters here because it is roughly thirty times the transverse mechanical dispersion and spreads the plume by about five layers over the twenty year simulation.  Results are shown after 8, 12, and 20 years (figures \ref{fig:ex-gwt-mt3dms-p08-8yrs} - \ref{fig:ex-gwt-mt3dms-p08-20yrs}).  In each of the selected years for which results are plotted, the two models produce plumes of nearly the same extent and shape.  The remaining difference is at the center of the plume, where \mf is slightly less concentrated than MT3D-USGS by about one contour interval in year 12, which is consistent with the \mf TVD scheme being somewhat more diffusive than the third-order TVD scheme used by MT3D-USGS.
