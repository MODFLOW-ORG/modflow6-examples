...
Simulated and analytical drawdown at three observation points are shown in figure~\ref{fig:ex-gwf-hwell-compare}\textit{A}. Every representation approaches \cite{zhan2001} as the cone of depression spreads over more cells. After 0.1 $d$ the WEL Package well is within 3.2 percent, and at 1 $d$ it is within 1.2 percent at all three points. The difference is largest early and at the point directly beyond the end of the screen, reaching 10 percent at 0.009 $d$, where drawdown is 0.20 $m$ and the cells have begun to expand. Discharge is imposed at the distribution the analytical solution assumes, so the discrepancy results from spatial and temporal discretization error and the no-flow boundaries at the edges of the model domain.
...
Drawdown in the plane of the well (layer 6) is mapped in figure~\ref{fig:ex-gwf-hwell-map}. The WEL Package well departs from the analytical solution only along the screen itself, where the grid resolves a line sink with cells 1.25 $m$ wide, and by a small amount over the whole map at 1 $d$ from the closed boundary. The MAW and CLN well representations are consistent, with more drawdown at the ends of the screen and less over its middle, which is the redistribution of discharge in figure~\ref{fig:ex-gwf-hwell-compare}\textit{B} carried into the aquifer.
